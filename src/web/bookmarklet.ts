...
export function buildBookmarkletJs(extras: Array<{ label: string; code: string; execution?: BookmarkletExecution }>): string {
  ...
  const injection = `setTimeout(function(){var _exbms=${extrasJson};if(_exbms.length>0){addSection('EXTRA');_exbms.forEach(function(bm,i){var _id='ex'+i;var _lbl=bm.label.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');addRow(_id,_lbl);function _fail(tx){setRow(_id,'\\u2715','#f87171',tx||'실패');}function _ok(tx){okRow(_id,tx||'완료');}try{var _done=false,_seen=false,_exec=bm.execution||{},_timeoutMs=_exec.timeoutMs||12000;function _timeout(){if(!_done){_done=true;(_seen?_fail('응답 없음'):_fail('실행 오류'));}}var _timer=setTimeout(_timeout,_timeoutMs);function _restore(){doc.body.appendChild=_ac;doc.body.append=_ap;}function _finish(fn,tx){if(_done)return;_done=true;clearTimeout(_timer);_restore();fn(tx);}function _watch(n){_seen=true;if(n&&n.tagName&&n.tagName.toLowerCase()==='script'){n.addEventListener('load',function(){if(_exec.completionTopic){setRow(_id,'\\u21bb','#facc15','실행 중');}else{_finish(_ok,'로드됨');}},{once:true});n.addEventListener('error',function(){_finish(_fail,'로드 실패');},{once:true});}}var _ac=doc.body.appendChild,_ap=doc.body.append;doc.body.appendChild=function(n){_watch(n);return _ac.call(this,n);};doc.body.append=function(){for(var _ai=0;_ai<arguments.length;_ai++)_watch(arguments[_ai]);return _ap.apply(this,arguments);};try{var _c=bm.code.replace(/^javascript:/,'');(0,eval)(_c);if(_exec.popupUrl){setRow(_id,'\\u21bb','#facc15','팝업 확인');}_seen=true;if(!_exec.completionTopic)_finish(_ok,'완료');}finally{if(!_exec.completionTopic)_restore();}}catch(_e){console.warn('[carol] extra:',bm.label,_e);_fail('실패');}});}},0);`;
  const marker = "})()";
